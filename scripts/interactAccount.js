...
async function getTokenBoundAccount() {
  ...
  return await ERC6551Registry.account(
...
async function sendFundsToTokenAccount() {
  try {
    ...
    console.log(
      `Token account has ${balanceBefore.toString()} ETH before transfer`
    );
    ...
    const tokenAccountBalance = ethers.formatEther(
      (await hre.ethers.provider.getBalance(computedAddress)).toString()
    );
...
async function transferToken() {
  try {
    ...
    const approveTxn = await ERC721Contract.approve(
    ...
    const transferTxn = await ERC721Contract.transferFrom(
      signer.address,
      tokenBoundAccountAddress,
      tokenId
    );
